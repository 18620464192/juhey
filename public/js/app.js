...
function renderData(data) {
    const container = document.getElementById('categories'); // 修改为 'categories'
...
function initSidebar() {
    ...
    sidebarToggle.innerHTML = '<img src="public/img/cd.png" alt="折叠侧边栏">';
    ...
    // const isCollapsed = storedState === 'true'; // 无存储时为 false  储存时true
...
function mergeData(allData) {
    return {
        categories: allData.flatMap(data => data.categories)
    };
}
    ...
    categoryFiles.map(file =>
        fetch(file)
            ...
            .catch(error => {
                ...
                return { categories: [] }; // 返回空数据避免中断
            })
    )
)
    .then(allData => mergeData(allData))
    .then(renderData)
    .then(initSearch)
    .catch(console.error);